...
@Table(
        indexes = {
                @Index(name = "pays_cogerant_id_idx", columnList = "nationalite"),
                @Index(name = "commune_residence_cogerant_id_idx", columnList = "commune_residence_id"),
                @Index(name = "commune_naissance_cogerant_id_idx", columnList = "commune_naissance_id"),
        }
)
public class CoGerant extends AbstractEntity{
    ...
    @OneToMany(fetch = LAZY, mappedBy = "coGerant", orphanRemoval = true)
    private Collection<GerantEntreprise> entreprises;
